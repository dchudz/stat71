...
library(coda)
library(plyr)
source("CompareStanSIR/Sir/drawbeta_logit.R")
source("Logit_rejection.R")
set.seed(908952)
...
stanFitLogit1D <- stan("CompareStanSIR/Stan/Logit1D.Stan", data = s71logit_dat, iter = nStanIterations, chains = 4)
stanFitLogit1D

betaSamplesStan <- extract(stanFitLogit1D)$beta1
betaSamplesSir <- drawbeta(5000, matrix(x), y, int=FALSE)
betaSamplesRej <- c(logit.rej(5000, matrix(x), y), F)	#could set prnt=F here
...
stanFitLogitMultiple <- stan("CompareStanSIR/Stan/LogitMultipleRegression.Stan",
